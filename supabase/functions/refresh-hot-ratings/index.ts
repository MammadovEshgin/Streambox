...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (raw.trim()) {
      body = JSON.parse(raw) as { batchSize?: number };
    }
  } catch {
    body = {};
  }
  ...
  try {
    ...
    await safeLogFunctionEvent(supabase, {
      functionName: "refresh-hot-ratings",
      eventType: "missing_omdb_secret",
      statusCode: 500,
      latencyMs: Date.now() - startedAt,
      errorMessage: "Missing OMDb server secret.",
    });
    return json({ error: "Missing OMDb server secret." }, 500);
  }

  try {
    const batchSize = parseBatchSize(body.batchSize);
    const imdbIds = await getHotRatingsCandidates(supabase, batchSize);

    if (imdbIds.length === 0) {
      await safeLogFunctionEvent(supabase, {
        functionName: "refresh-hot-ratings",
        eventType: "refresh_run",
        statusCode: 200,
        latencyMs: Date.now() - startedAt,
        metadata: {
          batchSize,
    ...
    for (const imdbId of imdbIds) {
      const result = await refreshRatingsForImdbId({
        supabase,
        imdbId,
        omdbApiKey,
        trigger: "schedule",
      ...
      await safeLogFunctionEvent(supabase, {
        functionName: "refresh-hot-ratings",
        eventType: "refresh_item_failed",
        statusCode: 502,
        imdbId,
        cacheSource: "omdb",
        errorMessage: result.warning ?? "Failed to refresh OMDb ratings.",
      });
    }

    const statusCode = failed > 0 && refreshed === 0 ? 502 : 200;
    await safeLogFunctionEvent(supabase, {
      functionName: "refresh-hot-ratings",
      eventType: "refresh_run",
      statusCode,
    ...
    await safeLogFunctionEvent(supabase, {
      functionName: "refresh-hot-ratings",
      eventType: "refresh_run_failed",
      statusCode: 500,
      latencyMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : "Failed to refresh hot ratings.",
    });

    return json(
      {
        error: error instanceof Error ? error.message : "Failed to refresh hot ratings.",
      },
      500,
    );
  }
